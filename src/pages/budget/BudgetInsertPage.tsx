import {
  Container,
  Typography,
  Box,
  CardContent,
  CardMedia,
} from "@mui/material";
import BudgetInputFormat from "../../components/ BudgetInputFormat";
// import DatePick from "../../components/ BudgetInputFormat/DatePick";
// BudgetMainPage에서 지명을 받아서
// 그에 맞는 사진 가져오고 띄우기

export default function BudgetInsertPage() {
  return (
    <>
      <Container
        maxWidth="xl"
        sx={{
          my: 11,
          width: "1440px",
          height: "750px",
          display: "flex",
          justifyContent: "center",
        }}
      >
        <Box
          sx={{
            display: "flex",
            justifyContent: "center",
            height: "700px",
            mt: 3,
          }}
        >
          <Box sx={{ marginRight: "20px" }}>
            <CardContent sx={{ p: 0 }}>
              <CardMedia
                component="image"
                image="https://rimage.gnst.jp/livejapan.com/public/article/detail/a/00/04/a0004721/img/ko/a0004721_parts_5fab2f30a98c3.jpg?20230830000000&amp;q=80"
                sx={{
                  width: "650px",
                  height: "450px",
                  borderRadius: "10px",
                }}
              ></CardMedia>
              <img src="data:image/png;base64,"></img>
              <Typography variant="h4" color="black" gutterBottom>
                Tokyo, Japan
              </Typography>
            </CardContent>
          </Box>
          <Box sx={{ width: "650px", height: "750px" }}>
            <BudgetInputFormat></BudgetInputFormat>
          </Box>
        </Box>
      </Container>
    </>
  );
}

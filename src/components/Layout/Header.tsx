            ...
            <Container maxWidth="xl">
                <Toolbar disableGutters>
                    ...
                    <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
                        <MenuButton onClick={() => navigate('/budget')}>예산으로 찾기</MenuButton>
                        <MenuButton onClick={() => navigate('/imageplace')}>사진으로 찾기</MenuButton>
                        <MenuButton onClick={() => navigate('/community')}>커뮤니티</MenuButton>
                    </Box>
                    {/* 화면 클때 LOGO */}
                    ...
                    <Box sx={{ flexGrow: 0 }}>
                        <Tooltip title="Open settings">
                            <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                                <AccountCircleIcon fontSize="large" sx={{ color: 'black' }}></AccountCircleIcon>
                            </IconButton>
                        </Tooltip>
                        <Menu
                            sx={{ mt: '45px' }}
                            ...
                            {isLoggedInState.loginStatus
                                ? settingsWithLogin.map((setting) => (
                                      <Typography
                                          ...
                                          key={setting.name}
                                      >
                                          <MenuListButton onClick={(event) => closeUserMenu(event, setting)}>
                                              {setting.name}
                                          </MenuListButton>
                                      </Typography>
                                  ))
